...
export async function GET(request, { params }) {
  try {
    ...
    const [user] = await db.select().from(users)
      .where(and(eq(users.id, id), eq(users.role, 'athlete'))).limit(1)
    ...
    const recentSessions = await db
      .select({
        session_id: session_athletes.session_id, avg_hr: session_athletes.avg_hr,
        max_hr: session_athletes.max_hr, calories: session_athletes.calories,
        trimp: session_athletes.trimp, training_effect: session_athletes.training_effect,
        time_z1_sec: session_athletes.time_z1_sec, time_z2_sec: session_athletes.time_z2_sec,
        time_z3_sec: session_athletes.time_z3_sec, time_z4_sec: session_athletes.time_z4_sec,
        time_z5_sec: session_athletes.time_z5_sec,
        session_name: training_sessions.name, start_datetime: training_sessions.start_datetime,
        duration_min: training_sessions.duration_min, status: training_sessions.status,
      })
    ...
    return NextResponse.json({ ...safeUser, profile: profile ?? null, sensor: sensor ?? null, recent_logs: logs, acwr: acwr ?? null, recent_sessions: recentSessions })
...
// Body: qualquer campo de users ou athlete_profiles
// ─────────────────────────────────────────────────────────────────────────────
export async function PUT(request, { params }) {
  try {
    ...
    // ── Atualiza perfil esportivo ─────────────────────────────────────────
    const profileUpdate = {}
    const profileFields = ['hr_max','hr_rest','hr_threshold','weight_kg','height_cm',
    ...
    // Recalcula zonas se hr_max foi alterado
...
// Não apaga do banco — apenas marca is_active=0 e status=inactive
// ─────────────────────────────────────────────────────────────────────────────
export async function DELETE(request, { params }) {
  try {
    ...
    // Soft delete — preserva histórico de treinos
