...
export async function GET(req) {
  ...
  try {
    // ─ Atleta independente: só as próprias sessões (INNER JOIN) ─
    if (role === 'athlete') {
      const rows = await db
...
export async function POST(req) {
  ...
  const buildRow = (start, groupId = null) => {
    ...
    return {
      ...
      recurrence_end_date:  groupId ? new Date(recurrence_end_date + 'T23:59:59-03:00') : null,
    }
  }
  ...
  try {
    ...
    const endDate = new Date(recurrence_end_date + 'T23:59:59-03:00')
    const base    = parseLocalDatetime(start_datetime)
    const baseH   = base.getUTCHours()    // Já é UTC correto após parseLocalDatetime
    const baseM   = base.getUTCMinutes()
    const groupId = randomUUID()
    const rows    = []
    // cursor em UTC midnight do dia local (UTC-3)
    const cursor  = new Date(Date.UTC(base.getUTCFullYear(), base.getUTCMonth(), base.getUTCDate()))

    while (cursor <= endDate) {
      // dia da semana no fuso local (UTC-3)
      const localDay = new Date(cursor.getTime() - 3 * 60_000).getUTCDay()
      if (days.includes(localDay)) {
        const startFixed = new Date(Date.UTC(
          cursor.getUTCFullYear(), cursor.getUTCMonth(), cursor.getUTCDate(),
          baseH, baseM, 0, 0
        ))
        rows.push(buildRow(startFixed, groupId))
      }
      cursor.setUTCDate(cursor.getUTCDate() + 1)
    }

    if (rows.length === 0) {
      return NextResponse.json({ error: 'Nenhuma ocorrencia gerada para os dias selecionados' }, { status: 400 })
    }
